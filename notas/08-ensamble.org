...
En el contexto de modelado predictivo nos interesa estimar la relación que existe entre atributos $x$ y la respuesta de interés $y$ por medio de una $f: \mathcal{X} \mapsto \mathcal{Y}$. Dicho estimador, lo denotamos por $\hat f_n$ haciendo énfasis en que ha sido construido con una muestra de tamaño $n$ al minimizar una función de pérdida adecuada.

#+REVEAL: split
Para cada uno de los $B$ conjuntos de entrenamiento estimamos $\hat f_n^{(b)}$ para poder hacer predicciones por medio de
\begin{align}
...
de los modelos $\hat f_n^{(b)}$ y tomar la clase con mas votos dentro del conjunto de
$B$ predictores.

** Error de generalización

- Usando /bagging/ entrenamos con cada uno de los conjuntos de datos remuestreados.
...
- El consenso de votos tiende a ser robusto contra sobre-ajustar y si utilizamos una $B$  (el número de árboles) ta seuficientemente grande estabilizamos la variabilidad del error de generalización. 
...
- Los bosques aleatorios son uno de los métodos mas generales de predicción.
