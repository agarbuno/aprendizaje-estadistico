...
- [[#otros-modelo-discriminantes][Otros modelo discriminantes]]
...
#+REVEAL: split
Usualmente resolvemos buscando poder predecir la ~probabilidad~ de que $X$ pertenezca a la categoría $\mathcal{C}$.
...
** El problema
La respuesta del modelo lineal no la podemos interpretar como una probabilidad.
...
Antes habíamos considerado predictores multiclase. Lo mismo podríamos pensar para una predicción en múltiples clases.
...
#+REVEAL: split
Por supuesto un ~predictor binario~ no es apropiado y necesitamos considerar otros modelos (que veremos mas adelante) como ~Regresión logística multiclase~ o ~Análisis Discriminante~. 
...
#+BEGIN_NOTES
La función $\sigma(x) = e^x / (1 + e^x)$  se conoce como ~función logística~. Análogamente, la función $\sigma(x) = 1/ (1 + e^{-x})$ se conoce como la ~función sigmoide~. Son... lo mismo.
#+END_NOTES
...
La proporción de ~errores~ para la clase ~No~ se le llama ~Tasa de Falsos Positivos~ (apróx. 0.2%).
...
La proporción de ~errores~ para la clase ~Si~ se le llama ~Tasa de Falsos Negativos~ (apróx. 73%).
...
- Si consideramos un modelo Guassiano con $\Sigma_k$ una matriz diagonal tenemos el ~clasificador Bayesiano ingenuo~.
...
- Con regresión logística aprendemos $\mathbb{P}(Y|X)$ (que se conoce como ~aprendizaje discrimnante~).
- Con LDA aprendemos $\mathbb{P}(X,Y)$ (que se conoce como ~aprendizaje generativo~).

#+BEGIN_NOTES
En la práctica los resultados son muy similares. 
#+END_NOTES
